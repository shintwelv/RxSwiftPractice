...
class WindowViewController: UIViewController {
    ...
    private lazy var descLabel: UILabel = {
        let label = UILabel()
        label.text = "\(groupNumber)개의 값 혹은 \(randomNumber)초 동안 입력된 값을 나누어 출력합니다"
    ...
    private func configureDataBinding() {
        numberObservable
            .window(timeSpan: .seconds(randomNumber), count: groupNumber, scheduler: MainScheduler.instance)
            .subscribe(
                onNext: { [weak self] observable in
