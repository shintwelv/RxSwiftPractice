...
class StartWithViewController: UIViewController {
    ...
    private lazy var descLabel: UILabel = {
        let label = UILabel()
        label.text = "GO버튼을 누르면 -3, -2, -1, 0 부터 방출합니다"
    ...
    private func configureAutoLayout() {
        ...
        NSLayoutConstraint.activate([
            ...
            self.goButton.widthAnchor.constraint(equalToConstant: 60),
            self.goButton.heightAnchor.constraint(equalToConstant: 60),
            self.goButton.topAnchor.constraint(equalTo: self.buttonStackView.bottomAnchor, constant: 10),
            self.goButton.centerXAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.centerXAnchor),
            
            self.touchHistoryLabel.topAnchor.constraint(equalTo: self.goButton.bottomAnchor, constant: 10),
